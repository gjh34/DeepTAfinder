...
def contrastive_loss(embedding, label, temp=0.3):
...
class BCEFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, weights):
        ...
        bce_loss = F.binary_cross_entropy_with_logits(inputs, targets.float(), weight=weights, reduction='none').to(device)
        pt = torch.exp(-bce_loss).to(device)
        focal_loss = (self.alpha * (1-pt)**self.gamma * bce_loss).to(device)
